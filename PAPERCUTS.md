...
- 2026-09-20 · PR #1 (FINANCE_ALERTS / meal-plan default-entry export fixes) merged into `integration` **without** `npm run check:boot` ever running — both launch paths failed in that cloud sandbox (see the two entries above) and were substituted with a hand-rolled Playwright probe (explicit `executablePath` + `--no-proxy-server`) that covered the same ground (boot completes, sign-in gate lifts, the two previously-broken code paths execute cleanly) but is not the standing automated check · Flagging so a future session doesn't assume this PR got the standard boot-check pass — if the sandbox's Playwright browser revision ever gets reconciled with what `scripts/check-boot.mjs` expects, worth re-running it once for real against this change · Live/Tableplan
