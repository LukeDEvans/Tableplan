...
export function createFollowingEngine({ id = "naive-v0", window = 4, backWindow = 3, lostAfter = 4, historySize = 8 } = {}) {
  ...
  const api = {
    ...
    push(event) {
      ...
      // Forward window: correct note, or jump past missed/skipped notes.
      for (let i = cursor; i < Math.min(cursor + window, expected.length); i++) {
        if (hit(expected[i].midis, midi)) {
          const skipped = i - cursor;
          lastPosition = expected[i].position; cursor = i + 1; record(true);
          return st(true, skipped);
        }
      }
      // Backward window: a restart or repeat of a recently-passed note.
      ...
      // Wrong / extra note: no advance, confidence erodes.
      record(false);
      return st(false);
    },
    reset(at) {
