const { scanArticleFromImages } = require("../../article-scan");
...
exports.handler = async (event) => {
  ...
  try {
    const result = await scanArticleFromImages(payload.images || []);
    return jsonResponse(200, { article: result.article, rawText: result.rawText, model: result.model });
  } catch (error) {
    return jsonResponse(500, { error: error.message || "Article scan failed." });
  }
};
