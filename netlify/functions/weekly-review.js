...
exports.handler = async (event) => {
  ...
  if (!anthropicKey || !serviceKey || !resendKey) {
    const missing = [
      !anthropicKey && "ANTHROPIC_API_KEY",
      !serviceKey && "SUPABASE_SERVICE_ROLE_KEY",
      !resendKey && "RESEND_API_KEY"
    ].filter(Boolean);
    return jsonResponse(200, { skipped: true, error: `Missing configuration: ${missing.join(", ")}` });
  }

  try {
    const appState = await loadStateFromSupabase(serviceKey);
    if (!appState) return jsonResponse(500, { error: "Could not load app state from Supabase." });
...
Output ONLY the HTML body content (start with a <div> or <table>, no doctype/html/head/body tags).${prefsSection}`,
    ...
    return jsonResponse(200, { ok: true, sentTo: to, subject });
  } catch (error) {
