...
// IndexedDB key. Large content (scores, recordings, article bodies, audio…) is
// referenced by a stable `blobId`; the BlobAsset catalog record carries content
// identity (sha-256 hash, size, MIME), ownership (owned vs cached), provenance,
// and a list of physical `locations` (this device's IndexedDB, an external URL,
// a cloud object, a future home-server). This is what lets bytes move between
// devices/backends without rewriting domain data.
//
// Domain-neutral: extracted verbatim from Cadence (music/blob.js) as Phase 0 of
// the local-first foundation. Contains no music-specific concepts.
//
 ...
 *   locations: [{kind:"idb", key} | {kind:"url", url} | {kind:"cloud", bucket, path} | …]
 ...
 *   "remote"  — no local bytes, but a fetchable location (url / cloud / home-server) exists
