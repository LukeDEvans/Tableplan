...
async function handleIcsProxy(url, response) {
  ...
  // Plan-ICS parsing lives in the shared ESM module calendar/ics.js (one tested
  // parser for ICS + Amion). Dynamic-import it from this CommonJS server; Node
  // caches the module after the first call.
  const { parsePlanIcs } = await import("./calendar/ics.mjs");
...
// parsePlanIcs + its ICS date/RRULE/tz helpers moved to ./calendar/ics.mjs
// (imported in handleIcsProxy). One tested parser for ICS + Amion feeds.
