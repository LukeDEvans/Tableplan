...
| **music** | `music-streaming.js`, `music-provider-*.js` (applemusic/internetarchive), `music-canonical.js`, `music-library*.js`, `music-source-resolver.js`, `music-tags.js`, `media-provider-music.js` | `"media"` (music tab) | `state.musicLibrary`, `state.appleMusic`, media state |
